...
Local checks cover TypeScript, lint, production build, 41 unit/database tests, 14 browser workflows and two built-PWA browser tests. Viewports include 390×844, 744×1133, 1133×744 and 1440×900. Browser checks include keyboard focus, doubled text, reduced motion, persisted theme, sign-out, access revocation, static-cache privacy, offline navigation and update lifecycle.
...
5. On a later release, confirm a normal refresh loads the latest app without an update popup. Repeat the layout/install checks on iPhone.
