...
try {
  role =
    JSON.parse(Buffer.from(secret?.split(".")[1] ?? "", "base64url").toString())
      .role ?? "";
} catch {
  role = "";
}
if (!secret?.startsWith("sb_secret_") && role !== "service_role")
  throw new Error(
    "SUPABASE_SECRET_KEY is not an accessible server credential (publishable, absent or masked).",
  );
...
if (result.error)
  throw new Error("Server database credential verification failed.");
console.log(
  JSON.stringify({
