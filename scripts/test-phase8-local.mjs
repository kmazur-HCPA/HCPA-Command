...
export async function testPhase8({
  admin,
  owner,
  outsider,
  users,
  ...
  const snapshot = await owner.rpc("export_workspace");
  ...
  for (const table of [
    ...
    "activity_log",
    "app_memberships",
    "cora_conversations", "cora_turns", "cora_activity", "cora_mcp_activity",
  ]) {
    ...
    assert.equal(source.count, backup.counts[table]);
    assert(backup[table].every((row) => row.user_id === ownerId));
  }
  const foreign = await outsider.rpc("export_workspace");
  assert.equal(foreign.error, null);
  assert.equal(foreign.data.work_items.length, 0);
  const search = await owner.rpc("search_work", { query_text: "original" });
  assert.equal(search.error, null);
  assert(search.data.length > 0);
  assert.equal(
    (await outsider.rpc("search_work", { query_text: "original" })).data.length,
    0,
  );
  const token = (await owner.auth.getSession()).data.session.access_token;
  const download = (id) =>
    fetch(`${url}/functions/v1/library`, {
      method: "POST",
      headers: {
  ...
  const originals = [];
  for (const version of backup.library_versions.filter(
    (v) => v.state === "ready",
  )) {
    const response = await download(version.id);
    ...
    originals.push({
      version,
      bytes,
      path: `${ownerId}/${version.document_id}/${version.id}`,
    });
  }
 ...
 foreach table_name in array array['cora_mcp_activity','cora_activity','cora_turns','cora_conversations','library_versions','journal_revisions','work_items','activity_log','user_preferences','app_memberships'] loop
 execute format('delete from public.%I where user_id=$1',table_name) using '${ownerId}'::uuid;
 end loop;
 foreach table_name in array array['app_memberships','user_preferences','work_items','journal_revisions','library_versions','activity_log','cora_conversations','cora_turns','cora_activity','cora_mcp_activity'] loop
 ...
 execute format('insert into public.%I (%s) select %s from jsonb_populate_recordset(null::public.%I,(select value->$1 from recovery_payload))',table_name,cols,cols,table_name) using table_name;
  ...
  for (const original of originals) {
    const restored = await admin.storage
      .from("command-library")
      .upload(original.path, original.bytes, {
        contentType: "application/octet-stream",
      });
    assert.equal(restored.error, null);
  }
  const recovered = await owner.rpc("export_workspace");
  assert.equal(recovered.error, null);
  for (const table of Object.keys(backup.counts))
    assert.deepEqual(recovered.data[table], backup[table], `restored ${table}`);
  for (const original of originals) {
    const response = await download(original.version.id);
    assert.equal(response.status, 200);
    assert.equal(
      createHash("sha256")
  ...
  console.log(
    JSON.stringify({
      ...
      scope:
        "disposable local application database and originals; existing Auth identities retained",
      records: backup.work_items.length,
      history: backup.journal_revisions.length,
      originals: originals.length,
      duration_ms: Math.round(performance.now() - started),
      result: "pass",
    }),
  );
  // Representative growth, rolled back completely. Two owners, 15,000 records, authenticated RLS path.
 ...
 rollback;`);
  ...
  await writeFile("test-results/search-query-plans.txt", timings);
  // RLS deliberately evaluates ownership before non-leakproof full-text operators.
  // PostgreSQL can therefore prefer an owner-filtered scan over the GIN index.
  // Gate actual authenticated latency, not a particular planner decision.
  const elapsed = [...timings.matchAll(/"Execution Time": ([\d.]+)/g)].map(
    (match) => Number(match[1]),
  );
  assert.equal(elapsed.length, 3);
  assert(
    elapsed.every((ms) => ms < 1500),
    "Search exceeds the 1.5 second local growth-test gate",
  );
  console.log(
    JSON.stringify({
      event: "search_growth_plan",
      records: 15000,
      owned: 10000,
      selective_filter_ms: elapsed[0],
      ranked_selective_ms: elapsed[1],
      ranked_common_ms: elapsed[2],
      gin_used: timings.includes("work_search"),
      scope: "local PostgreSQL with RLS; not production network latency",
    }),
  );
}
