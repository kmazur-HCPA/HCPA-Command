...
describe('Work Day calendar endpoint',()=>{
 it('returns a minimal owner-scoped agenda and excludes cancellations and declined events',async()=>{
  ...
  const response=await handleMicrosoft(request('calendar?date=2026-09-22&range=workweek'),server);
  ...
  expect(graphCalls[0]?.searchParams.get('startDateTime')).toBe('2026-09-21T04:00:00.000Z');
  expect(graphCalls[0]?.searchParams.get('endDateTime')).toBe('2026-09-26T04:00:00.000Z');
