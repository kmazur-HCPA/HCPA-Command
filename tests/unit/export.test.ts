...
describe("portable verified exports", () => {
  it("escapes CSV cells and prevents spreadsheet formulas without changing JSON", () => {
    expect(csvCell('=HYPERLINK("bad")')).toBe('"\'=HYPERLINK(""bad"")"');
    expect(csvCell("a,b\nc")).toBe('"a,b\nc"');
  });
  it("rejects mismatched counts, cross-owner records and broken relationships", () => {
    const s = snapshot();
    s.counts.work_items = 1;
    expect(() => validateSnapshot(s)).toThrow(/count mismatch/);
    s.work_items = [
      {
        ...newItem("task", uid),
        original_body: "",
    ...
    expect(() => validateSnapshot(s)).toThrow(/relationship/);
    s.work_items[0]!.project_id = null;
    s.work_items[0]!.user_id = "other";
    expect(() => validateSnapshot(s)).toThrow(/ownership/);
  });
  it("builds a readable archive whose entity counts reconcile", async () => {
    const s = snapshot();
    const client = {
      rpc: () => ({ abortSignal: async () => ({ data: s, error: null }) }),
  ...
  it("never creates an archive after cancellation", async () => {
    ...
    const client = {
      rpc: () => ({
        abortSignal: async () => ({ data: snapshot(), error: null }),
      }),
    } as unknown as AppClient;
    await expect(
      createWorkspaceExport(client, controller.signal, vi.fn()),
    ).rejects.toThrow();
  });
  it("verifies original bytes and refuses corrupted originals", async () => {
    const s = snapshot(),
      bytes = new TextEncoder().encode("Verified original");
    const document = {
      ...newItem("library", uid),
      original_body: "",
      version: 1,
    ...
    s.counts.library_versions = 1;
    let corrupt = false;
    const client = {
      rpc: () => ({ abortSignal: async () => ({ data: s, error: null }) }),
      functions: {
        invoke: async () => ({
          data: new Blob([corrupt ? "tampered" : bytes]),
          error: null,
        }),
      },
    } as unknown as AppClient;
    ...
    const files = unzipSync(new Uint8Array(await result.blob.arrayBuffer()));
    expect(
      strFromU8(files[`originals/${s.library_versions[0]!.id}/original.txt`]!),
    ).toBe("Verified original");
    corrupt = true;
    await expect(
      createWorkspaceExport(client, new AbortController().signal, vi.fn()),
