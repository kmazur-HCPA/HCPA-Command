  ...
  return measured("work.search", async () => {
    const { data, error } = await client
      .rpc("search_work", {
        ...
        module_filter: filters.module || undefined,
        status_filter: filters.status || undefined,
        archive_filter: filters.archive,
        tag_filter: filters.tag.trim() || undefined,
        page_offset: offset,
      })
      .abortSignal(signal);
    if (error)
      throw new Error("Search could not finish. Reconnect and try again.");
