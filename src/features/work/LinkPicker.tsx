...
export function LinkPicker({client,kind,label,value,onChange,disabled}:{client:AppClient;kind:Kind;label:string;value:string|null;onChange:(id:string|null)=>void;disabled:boolean}) {
 ...
 useEffect(()=>{let alive=true;const timer=setTimeout(()=>{void listChoices(client,kind,query).then(r=>{if(alive){setRows(r);setError('')}}).catch(()=>{if(alive)setError('Could not load link choices')})},200);return()=>{alive=false;clearTimeout(timer)}},[client,kind,query])
