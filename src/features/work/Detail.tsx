...
export function Detail({client,userId,id,onClose,modal=false,onChanged}:{client:AppClient;userId:string;id:string;onClose:()=>void;modal?:boolean;onChanged?:()=>void}) {
 ...
 const content = modal&&editing&&item ? <Editor embedded client={client} userId={userId} kind={item.kind} item={item} onClose={()=>setEditing(false)} onSaved={saved}/> : <section>{!modal&&<button onClick={onClose}>Back to list</button>}{error&&<p role="alert" className="error-message">{error}</p>}{!item&&!error&&<p role="status">Loading record…</p>}{item&&<>
  <div className="section-heading"><div><p className="eyebrow">{item.kind}{item.archived?' · Archived':''}</p><h1 id="detail-title" tabIndex={-1}>{item.title}</h1></div><button onClick={()=>setEditing(true)}>Edit</button></div>
  {item.kind==='task'&&<TaskMetadata item={item}/>}
 ...
 return modal ? <dialog ref={dialog} className="record-dialog task-detail-dialog" aria-label={editing?"Edit task":"Task details"} onCancel={event=>{event.preventDefault();if(!editing)onClose()}}>{!editing&&<div className="task-dialog-close"><button onClick={onClose} aria-label="Close task details">×</button></div>}{content}</dialog> : content
}
