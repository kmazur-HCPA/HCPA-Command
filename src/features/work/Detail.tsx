...
export function Detail({client,userId,id,onClose}:{client:AppClient;userId:string;id:string;onClose:()=>void}) {
 ...
 useEffect(()=>{if(kind!=='journal')return;let alive=true;revisions(client,id,historyOffset).then(rows=>{if(alive){setHistory(old=>historyOffset?[...old,...rows]:rows);setHistoryMore(rows.length===20)}}).catch(caught=>{if(alive)setError((caught as Error).message)});return()=>{alive=false}},[client,id,kind,historyOffset,revision])
 ...
 return <section><button onClick={onClose}>Back to list</button>{error&&<p role="alert" className="error-message">{error}</p>}{!item&&!error&&<p role="status">Loading record…</p>}{item&&<>
  ...
  {['project','initiative','person','journal','task'].includes(item.kind)&&<section><h2>{item.kind==='journal'?'Resulting tasks':'Related work'}</h2>{!related.length?<p className="muted">No current linked work.</p>:<ul>{related.map(row=><li key={row.id}><a href={`/?record=${row.id}`}>{row.title}</a> · {row.kind} · {row.status}</li>)}</ul>}{more&&<button onClick={()=>setOffset(related.length)}>More related work</button>}</section>}
  {item.kind==='journal'&&<><button onClick={()=>setCreateTask(true)}>Create resulting task</button><details><summary>Original entry</summary><p className="full-text">{item.original_body}</p></details><details><summary>Revision history</summary>{history.map(row=><article key={row.id} className="revision"><h3>Version {row.version} · {displayDate(null,row.created_at)}</h3><p>{row.snapshot.title}</p><p className="full-text">{row.snapshot.body}</p></article>)}{historyMore&&<button onClick={()=>setHistoryOffset(history.length)}>Earlier revisions</button>}</details></>}
