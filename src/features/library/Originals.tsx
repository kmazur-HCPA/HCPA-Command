...
export function Originals({client,item}:{client:AppClient;item:WorkItem}){
 ...
 {busy&&<><progress max={100} value={progress} aria-label="Upload progress"/><p role="status">{progress<100?`Uploading · ${progress}%`:'Transferred · validating and saving…'}</p></>}
 ...
 <ul className="record-list">{rows.map(row=><li key={row.id}><strong>{row.filename}</strong><p className="muted small">{new Date(row.created_at).toLocaleString()} · {Math.ceil(row.size_bytes/1024)} KiB · {row.state==='ready'?'Verified original':'Pending — retry required'}</p>{row.state==='ready'?<button disabled={busy} onClick={()=>{setBusy(true);setError('');void downloadOriginal(client,row).catch(caught=>setError((caught as Error).message)).finally(()=>setBusy(false))}}>Download original</button>:<button disabled={busy||item.archived} onClick={()=>retry(row)}>Recover pending upload</button>}<details><summary>Integrity</summary><code className="file-hash">SHA-256 {row.sha256}</code></details></li>)}</ul>{more&&<button onClick={()=>setOffset(rows.length)}>Earlier originals</button>}
 </section>
}
